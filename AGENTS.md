# ZORIXEL AIOS Workspace Rules (`AGENTS.md`)

> **MANDATORY SYSTEM PROMPT DIRECTIVE FOR ALL AI AGENTS & CHAT SESSIONS:**  
> This file is loaded directly into the system prompt window (`<user_rules>`) on every chat startup across all workspace sessions. Every AI agent, subagent, and LLM session MUST strictly adhere to every rule documented below without exception.
...
- **Graphify-First Context Discovery**: ALWAYS run `python scripts/graphify_runner.py` or inspect graphify AST context at task startup before making plans or edits.
- **Zero Prompting to Move Forward**: Do NOT rush planning or push toward execution waves. NEVER suggest moving forward to code, never prompt the user to start writing code, and never ask "Should we start coding?".
...
6. **MCP Engineering Pipeline**: `/build-mcp-server` -> `/build-mcpb` -> `/build-mcp-app` -> `/mcp-integration`.
7. **Composio SaaS Integration**: `/composio` (1000+ apps) -> `/gws-automation-engine` -> `/notion-sync` -> `/zero-paywall-client-os`.
8. **React Video & Motion Engine**: `/canvas-design` -> `/gsap-animation` -> `/remotion` -> `/carousel-render`.
9. **Autoresearch & Observability**: `/storm-research-project` -> `/smart-explore` -> `/autoresearch-manage` -> `/graphify`.
10. **Visual Handoff & Deck Engine**: `/excalidraw-diagram` -> `/frontend-slides` -> `/interactive-operator-guide-generator` -> `/wowerpoint`.
