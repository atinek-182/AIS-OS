...
### Via Antigravity CLI Custom Source

Install directly from this public GitHub repo. Run this in your terminal:

```text
agy plugin install frontend-slides@zarazhangrui/frontend-slides
```

Then use it by typing `/frontend-slides` in the Antigravity chat.

### Antigravity Manual Installation

Copy the skill files to your Antigravity skills directory:

```bash
# Create the skill directory
mkdir -p ~/.gemini/config/skills/frontend-slides/scripts

# Copy the user-facing skill files
cp SKILL.md STYLE_PRESETS.md viewport-base.css html-template.md animation-patterns.md ~/.gemini/config/skills/frontend-slides/
cp -R bold-template-pack ~/.gemini/config/skills/frontend-slides/
cp scripts/extract-pptx.py scripts/deploy.sh scripts/export-pdf.sh ~/.gemini/config/skills/frontend-slides/scripts/
```

Or clone directly:

```bash
git clone https://github.com/zarazhangrui/frontend-slides.git ~/.gemini/config/skills/frontend-slides
```
