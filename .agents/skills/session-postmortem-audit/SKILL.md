...
3. **Auto-Evolve Skill Instructions**: Review the findings of `/roast` against `.agents/skills/session-postmortem-audit/SKILL.md` (and any related active skills). Update the skill file itself with newly discovered edge cases, shell rules, or prompt safeguards using `replace_file_content` or `write_to_file`.
4. **Register & Log**:
...
5. **Persist System Learnings**: Suggest running `/improve-system` to merge these lessons into global AIOS memory.
