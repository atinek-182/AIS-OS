...
A skill is a reusable set of instructions that tells AI agents how to handle a specific task. Skills live in `.agents/skills/[skill-name]/SKILL.md` inside your project. When you type `/skill-name` or describe what you need in natural language, Antigravity loads the skill's instructions and follows them.

Think of skills as SOPs for AI agents. Instead of re-explaining a workflow every conversation, you write it once and invoke it forever.
...
**Round 2: Trigger**
*Why this matters: The `description` field is how Antigravity decides whether to load your skill. Bad trigger words mean Antigravity never uses it. Too broad means Antigravity fires it when you don't want it.*

- What would someone say to trigger this? (Get 2-3 natural language phrases)
- Should it be user-only (`/slash-command`), Antigravity-auto-invocable, or both?
- Does it accept arguments? If so, what? (e.g., a topic, a URL, a file path)

**Round 3: Step-by-Step Process**
*Why this matters: Antigravity follows instructions literally. Vague steps produce vague results. Specific steps produce consistent output every time.*

- Walk me through exactly what should happen from trigger to output. What's step 1? Step 2? Keep going.
- For each step: Does Antigravity do it directly, or delegate to a subagent/script?
...
- **Reference skills** add knowledge Antigravity applies to current work without performing an action. Examples: coding conventions, API patterns, style guides.
...
- `disable-model-invocation: true` -- Set if the skill has side effects (file generation, API calls, costs money). Prevents Antigravity from auto-invoking.
...
2. **Step-by-step workflow** -- Numbered steps. Each step tells Antigravity exactly what to do.
...
If your skill needs detailed reference docs, examples, or scripts, add them alongside SKILL.md in the same directory. Reference them from SKILL.md so Antigravity knows they exist. Supporting files are NOT loaded automatically -- they load only when Antigravity needs them. See [reference.md](reference.md) for the full pattern.

**Step 5: Document in GEMINI.md**

Your project's `GEMINI.md` file is where Antigravity loads project-wide instructions every conversation. After creating a skill, add a brief entry so you (and your team) know what's available:
...
This isn't required for the skill to work, but it keeps your project organized and helps Antigravity understand how skills fit into your broader workflow.

**Step 6: Test**

Test both invocation methods:

1. **Natural language** -- Say something matching the description. Does Antigravity load the skill?
...
**File:** `.agents/skills/meeting-notes/SKILL.md`
...
- [ ] No vague instructions -- every step tells Antigravity exactly what to do
