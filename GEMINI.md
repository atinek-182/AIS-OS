...
- **Mandatory Quality Evaluation & Synchronous Code-Doc Sync**: Whenever the user suggests adding, modifying, or removing a feature, rule, or asset: FIRST evaluate and check whether it is architecturally sound and good before applying changes. When confirmed, execute changes across BOTH code and documentation simultaneously so code and docs never drift.
- **Graphify-First Context Discovery (Mandatory Startup Rule)**: Just like process skills (`using-superpowers`), ALWAYS use `graphify` (`graphify query` or `python scripts/graphify_runner.py`) at the start of tasks to extract codebase structure, AST connections, and context effortlessly before making plans or edits.
