...
GWS_JS_PATH = r"C:\Users\HP\AppData\Roaming\npm\node_modules\@googleworkspace\cli\run.js"

def run_gws(args, payload_obj=None):
    cmd = ["node", GWS_JS_PATH] + args
    if payload_obj is not None:
        cmd.extend(["--json", json.dumps(payload_obj)])
...
def main():
    print("Creating Google Sheet Vashishthya-01Master-OS on your personal Google account...")
    ...
    create_payload = {
        ...
        "sheets": [
            {"properties": {"title": "Scholars Intake", "gridProperties": {"frozenRowCount": 1}}},
            {"properties": {"title": "Services Catalog & Pricing", "gridProperties": {"frozenRowCount": 1}}},
            {"properties": {"title": "Monthly Revenue Ledger", "gridProperties": {"frozenRowCount": 1}}},
            {"properties": {"title": "Team Task Distribution", "gridProperties": {"frozenRowCount": 1}}}
        ]
    }
    
    sheet_res = run_gws(["sheets", "spreadsheets", "create"], create_payload)
    spreadsheet_id = sheet_res["spreadsheetId"]
    spreadsheet_url = sheet_res["spreadsheetUrl"]
    
    print(f"\n========================================================")
    print(f"SUCCESS! Spreadsheet Created Successfully.")
    ...
    run_gws(["sheets", "spreadsheets", "values", "update", "--params", params_intake], {"values": intake_data})
    print("Populated Tab 1: Scholars Intake (18 formatted columns & sample rows)")
    ...
    run_gws(["sheets", "spreadsheets", "values", "update", "--params", params_cat], {"values": catalog_data})
    print("Populated Tab 2: Services Catalog & Pricing (7 canonical services matrix)")
    ...
    run_gws(["sheets", "spreadsheets", "values", "update", "--params", params_rev], {"values": revenue_data})
    print("Populated Tab 3: Monthly Revenue Ledger (August 2026 financial summary)")
    ...
    run_gws(["sheets", "spreadsheets", "values", "update", "--params", params_team], {"values": team_data})
    print("Populated Tab 4: Team Task Distribution (Staff workload & balance follow-ups)")
    
    print("\n========================================================")
    print("ALL 4 TABS CREATED & POPULATED SUCCESSFULLY!")
    print(f"Spreadsheet URL: {spreadsheet_url}")
    print("========================================================\n")
